...
function procedure_update_outer(table_name, arr) {
    let pk_str = arr[0].split(" ")[2];
    let clsName = "Cls" + table_name.split("TShopee")[1];
    return `Connection conn = fac.createConnection(); try { PreparedStatement stmt = conn.prepareStatement("${procedure_str_2(procedure_update(table_name), arr.length)}"); ${procedure_update_inner(arr)} stmt.executeUpdate(); System.out.println(String.format("Successfully updated product with ${capitalize(remove_underscore(pk_str))} %s", cb.get${capitalize(pk_str)}())); } catch (SQLException e) { e.printStackTrace(); }`.replace(/; /g, ";\n");
}
...
for(let key in java_dict){
    let table_name = key;
    let procedure_update = java_dict[table_name];
    let arr = new_arr(procedure_update);
    console.log(procedure_update_outer(table_name, arr));
    console.log();
}